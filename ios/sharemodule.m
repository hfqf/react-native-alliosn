...
@interface sharemodule ()<RCTBridgeModule>
@end
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [[UMSocialManager defaultManager] shareToPlatform: [self configPlatform: platformType]  messageObject:messageObject currentViewController:nil completion:^(id data, NSError *error) {
      ...
      if (error) {
        ...
        ret = @(error.code);
        callback( [[NSArray alloc] initWithObjects:ret,message, nil]);
      }else{
        if ([data isKindOfClass:[UMSocialShareResponse class]]) {
          ...
          callback( [[NSArray alloc] initWithObjects:ret,resp.message, nil]);
        }else{
          UMSocialLogInfo(@"response data is %@",data);
           ret = @(1);
          callback( [[NSArray alloc] initWithObjects:ret,@"分享失败", nil]);
        }
